...
public sealed class GeminiRequestTests
{
    ...
    public void FromPromptItReturnsWithConfiguration()
    {
        ...
        Assert.Equal(
            executionSettings.ResponseSchema.ToString(),
            request.Configuration.ResponseSchema.ToString()
        );
    ...
    public void JsonElementResponseSchemaFromPromptReturnsAsExpected()
    {
        ...
        {
            ResponseMimeType = "application/json",
            ResponseSchema = Microsoft.Extensions.AI.AIJsonUtilities.CreateJsonSchema(
                typeof(int),
                serializerOptions: GeminiRequest.GetDefaultOptions()
            )
        };
    ...
    public void KernelJsonSchemaFromPromptReturnsAsExpected()
    {
        ...
        Assert.Equal(executionSettings.ResponseMimeType, request.Configuration.ResponseMimeType);
        AssertDeepEquals(
            ((KernelJsonSchema)executionSettings.ResponseSchema).RootElement,
            request.Configuration.ResponseSchema.Value
        );
    }
    ...
    public void JsonNodeResponseSchemaFromPromptReturnsAsExpected()
    {
        ...
        var executionSettings = new GeminiPromptExecutionSettings
        {
            ...
            ResponseSchema = JsonNode.Parse(
                Microsoft.Extensions.AI.AIJsonUtilities.CreateJsonSchema(typeof(int)).GetRawText()
            )
        };
        ...
        Assert.NotNull(request.Configuration.ResponseSchema);
        Assert.Equal(
            JsonSerializer.SerializeToElement(executionSettings.ResponseSchema).GetRawText(),
            request.Configuration.ResponseSchema.Value.GetRawText()
        );
    }
    ...
    public void FromPromptItReturnsWithSafetySettings()
    {
        ...
        var executionSettings = new GeminiPromptExecutionSettings
        {
            SafetySettings = [new(GeminiSafetyCategory.Derogatory, GeminiSafetyThreshold.BlockNone)]
        };
        ...
        Assert.Equal(
            executionSettings.SafetySettings[0].Category,
            request.SafetySettings[0].Category
        );
        Assert.Equal(
            executionSettings.SafetySettings[0].Threshold,
    ...
    public void FromChatHistoryItReturnsWithConfiguration()
    {
        ...
        // Act
        var request = GeminiRequest.FromChatHistoryAndExecutionSettings(
            chatHistory,
            executionSettings
        );
    ...
    public void FromChatHistoryMultipleSystemMessagesItReturnsWithSystemMessages()
    {
        ...
        Assert.All(
            systemMessages,
            msg => Assert.Contains(request.SystemInstruction.Parts, p => p.Text == msg)
        );
    }
    ...
    public void FromChatHistoryTextAsTextContentItReturnsWithChatHistory()
    {
        ...
        Assert.Collection(
            ...
            c =>
                Assert.Equal(
                    chatHistory[2].Items.Cast<TextContent>().Single().Text,
                    c.Parts![0].Text
                )
        );
    }
    ...
    public void FromChatHistoryImageAsImageContentItReturnsWithChatHistory()
    {
        ...
        chatHistory.AddUserMessage(contentItems: [new ImageContent(imageAsBytes, "image/png")]);
        var executionSettings = new GeminiPromptExecutionSettings();
        ...
        Assert.Collection(
            ...
            c =>
                Assert.Equal(
                    chatHistory[2].Items.Cast<ImageContent>().Single().Uri,
    ...
    public void FromChatHistoryAudioAsAudioContentItReturnsWithChatHistory()
    {
        ...
        chatHistory.AddUserMessage(
            contentItems:
            [
                new AudioContent(new Uri("https://example-audio.com/file.wav"))
                {
                    MimeType = "audio/wav"
                }
            ]
        );
        chatHistory.AddUserMessage(contentItems: [new AudioContent(audioAsBytes, "audio/mp3")]);
        var executionSettings = new GeminiPromptExecutionSettings();
        ...
        Assert.Collection(
            ...
            c =>
                Assert.Equal(
                    chatHistory[2].Items.Cast<AudioContent>().Single().Uri,
                    c.Parts![0].FileData!.FileUri
                ),
            c =>
                Assert.True(
    ...
    public void FromChatHistoryPdfAsBinaryContentItReturnsWithChatHistory()
    {
        ...
        chatHistory.AddUserMessage(
            contentItems:
            [
                new BinaryContent(new Uri("https://example-file.com/file.pdf"))
                {
        ...
        chatHistory.AddUserMessage(
            contentItems: [new BinaryContent(pdfAsBytes, "application/pdf")]
        );
        var executionSettings = new GeminiPromptExecutionSettings();
        ...
        Assert.Collection(
            ...
            c =>
                Assert.Equal(
                    chatHistory[2].Items.Cast<BinaryContent>().Single().Uri,
                    c.Parts![0].FileData!.FileUri
                ),
            c =>
                Assert.True(
    ...
    public void FromChatHistoryCalledToolNotNullAddsFunctionResponse()
    {
        ...
        var toolCall = new GeminiFunctionToolCall(
            new GeminiPart.FunctionCallPart { FunctionName = "function-name" }
        );
        GeminiFunctionToolResult toolCallResult =
            new(toolCall, new FunctionResult(kernelFunction, expectedArgs));
        chatHistory.Add(
            new GeminiChatMessageContent(AuthorRole.Tool, string.Empty, "modelId", toolCallResult)
        );
        var executionSettings = new GeminiPromptExecutionSettings();
        ...
        // Assert
        Assert.Single(request.Contents, c => c.Role == AuthorRole.Tool);
        Assert.Single(request.Contents, c => c.Parts![0].FunctionResponse is not null);
        Assert.Single(
            request.Contents,
            c =>
                string.Equals(
                    c.Parts![0].FunctionResponse!.FunctionName,
                    toolCallResult.FullyQualifiedName,
    ...
    public void FromChatHistoryToolCallsNotNullAddsFunctionCalls()
    {
        ...
        chatHistory.Add(
            new GeminiChatMessageContent(
                ...
                "tool-message",
                "model-id",
                functionsToolCalls: [toolCallPart]
            )
        );
        chatHistory.Add(
            new GeminiChatMessageContent(
                ...
                "tool-message2",
                "model-id2",
                functionsToolCalls: [toolCallPart2]
            )
        );
        var executionSettings = new GeminiPromptExecutionSettings();
    ...
    public void AddFunctionToGeminiRequest()
    {
        ...
        var function = new GeminiFunction(
            "function-name",
        ...
        // Assert
        Assert.Collection(
            request.Tools!.Single().Functions,
    ...
    public void AddChatMessageToRequest()
    {
        ...
        Assert.Single(request.Contents, c => Equals(message.Role, c.Role));
    }
    ...
    public void CachedContentFromPromptReturnsAsExpected()
    {
        // Arrange
        var prompt = "prompt-example";
        var executionSettings = new GeminiPromptExecutionSettings { CachedContent = "xyz/abc" };
    ...
    public void ResponseSchemaAddsTypeToEnumProperties()
    {
        ...
        var properties = request
            .Configuration.ResponseSchema.Value.GetProperty("properties")
            .GetProperty("Movies")
            .GetProperty("items")
    ...
    public void FromPromptAndExecutionSettingsWithThinkingConfigReturnsInGenerationConfig()
    {
        ...
        // Assert
        Assert.Equal(
            executionSettings.ThinkingConfig.ThinkingBudget,
            request.Configuration?.ThinkingConfig?.ThinkingBudget
        );
    }

    [Fact]
    public void FromPromptAndExecutionSettingsWithIncludeThoughtsReturnsInGenerationConfig()
    {
        // Arrange
        var prompt = "prompt-example";
        var executionSettings = new GeminiPromptExecutionSettings
        ...
        Assert.Equal(
            executionSettings.ThinkingConfig.IncludeThoughts,
            request.Configuration?.ThinkingConfig?.IncludeThoughts
        );
    }
    ...
    [InlineData(false)]
    [InlineData(null)]
    public void FromPromptAndExecutionSettingsHandlesIncludeThoughtsValues(bool? includeThoughts)
    {
        // Arrange
        var prompt = "prompt-example";
        var executionSettings = new GeminiPromptExecutionSettings
        ...
        // Assert
        Assert.Equal(includeThoughts, request.Configuration?.ThinkingConfig?.IncludeThoughts);
    }
